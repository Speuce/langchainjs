...
class PrismaVectorStore extends vectorstores_1.VectorStore {
    ...
    async addModel(model) {
        return this.addDocument(
            new documents_1.Document({ pageContent: model[this.contentColumn], metadata: model })
        );
    }
    /**
    ...
    async addDocument(document) {
        ...
        return this.addVector(vector, document);
    }
    async addVector(vector, document) {
        const query = `
            UPDATE "${this.tableName}"
            SET ${this.vectorColumnName} = '[${vector.join(",")}]'::vector
            WHERE ${this.idColumn} = '${document.metadata[this.idColumn]}'
        `;
    ...
    _callbacks = undefined // implement passing to embedQuery later
        ...
        const results = await this.similaritySearchVectorWithScore(await this.embeddings.embedQuery(query), k, _filter);
